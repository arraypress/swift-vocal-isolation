...
public final class DemucsSeparator: @unchecked Sendable {
    ...
    public func separateSegment(_ mix: [[Float]]) async throws -> [[[Float]]] {
        ...
        var first = try await network(mix: mix, mag: mag, frames: frames, stats: stats)
        if verifying {
            let second = try await network(mix: mix, mag: mag, frames: frames, stats: stats)
            if second.spec != first.spec || second.time != first.time {
                recomputedSegments += 1
                let third = try await network(mix: mix, mag: mag, frames: frames, stats: stats)
                if third.spec == second.spec && third.time == second.time { first = second }
                else if !(third.spec == first.spec && third.time == first.time) {
                    throw VocalIsolationError.modelUnavailable("three runs of one segment disagreed; the GPU is not computing reliably")
                }
            }
        }
        return stems(fromSpec: first.spec, time: first.time, frames: frames)
    }
    ...
    private func network(mix: [[Float]], mag: [Float], frames: Int, stats: [Float]) async throws -> (spec: [Float], time: [Float]) {
        let S = Self.sources.count
    ...
    private func run(inputs: [String: NDArray], spec: inout NDArray, time: inout NDArray) async throws -> (spec: [Float], time: [Float]) {
